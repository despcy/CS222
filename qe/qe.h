...
private:
    ...
    bool isValiedDataFiltedByFilter(void* data);
    bool compare(void* lval,void* rval,CompOp op,AttrType type);
...
private:
    Iterator * iterator;
    std::vector<Attribute> attributesToProject;
    void project(void* input,std::vector<Attribute> inputAttr,void* output);
};
...
private:
    ...
    std::vector<string> bufferQueue;

    RC buildNextHash();

    RC searchAndAdd(void* const tuple);
    RC joinLeftAndRight(void *const leftTuple,int leftSize, void *const rightTuple,int rightSize, void *newTuple, int *tuplesize);
    RC readAttributeVal(void* attrVal, void* attrSize, int* tupleSize, void* const tuple,string attribute, vector<Attribute> attrs );
    //joinTuples func
...
private:
    ...
    RC min(void* data, Iterator *input);
    RC max(void* data, Iterator *input);
    RC count(void* data, Iterator *input);
    RC sum(void* data, Iterator *input);
    RC average(void* data, Iterator *input);
    RC readAttributeVal(void* attrVal, void* attrSize, int* tupleSize, void* const tuple,string attribute, vector<Attribute> attrs );
};
